...
| `--profile` | Preset bundle such as `quick`, `prod`, `pre-upgrade`, `incident`, `release`, `network`, `storage`, `admission`, `cost`, or `ci`. |
...
## Releases
- The GitHub release pipeline lives in `./.github/workflows/release.yml`.
- Push a semantic tag such as `v1.2.3` to trigger the first-class release pipeline automatically.
- The release workflow reruns the repository test/contract/runtime gates, builds raw binaries plus packaged archives for Linux, macOS, and Windows, generates `checksums.txt`, and publishes a GitHub Release with autogenerated notes.
- Tags containing `-alpha`, `-beta`, or `-rc` are published as prereleases automatically.
- You can also run the workflow manually from the Actions tab, but only when the selected ref is already a tag.
